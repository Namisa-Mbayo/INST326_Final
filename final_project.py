...
'''
#### now get <li> elements class contains cAJcNu
...
for i in range(0, len(listed_items)):
    ...
    #if the discount_prices list length is > 0 then this is a discounted item
    if len(dcnt_prices) > 0:
    
        for j in range(0, len(dcnt_prices)):
            #print(dcnt_prices[j].size())

            #NOT NEEDED: check if there is discount for current item
            #if dcnt_prices[j].get_attribute("innerHTML"):                
            #    print("discnt_price = ",dcnt_prices[j].get_attribute("innerHTML"))
            #else:
            #    print("discnt_price = 0")
